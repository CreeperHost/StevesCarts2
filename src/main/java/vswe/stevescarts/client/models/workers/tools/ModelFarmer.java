...
public class ModelFarmer extends ModelCartbase {
    ...
    public void buildModels() {
        ...
        PartDefinition anchor = modelPartData.addOrReplaceChild("anchor", CubeListBuilder.create(),
                PartPose.offset(-18.0f, 4.0f, 0.0f));
        ...
        for (int m = 0; m < 6; ++m) {
            ...
            PartDefinition nailAnchor = modelPartData.addOrReplaceChild("nailAnchor" + m, CubeListBuilder.create(),
            ...
            outers[m] = nailAnchor.bake(64, 64);
        }

        this.anchor = anchor.bake(64, 64);
    ...
    public void applyEffects(ModuleBase module, PoseStack matrixStack, MultiBufferSource rtb, float yaw, float pitch, float roll) {
        ...
        float offset = -22;
        float xPos = -18 + (float) Math.cos(mainAnchor.zRot + Math.PI) * offset;
        float yPos = 4 + (float) Math.sin(mainAnchor.zRot + Math.PI) * offset;
        anchor.x = xPos;
        anchor.y = yPos;
    ...
    public void renderToBuffer(@NotNull PoseStack poseStack, @NotNull VertexConsumer vertexConsumers, int light, int overlay, float red, float green, float blue, float alpha) {
        ...
        mainAnchor.render(poseStack, vertexConsumers, light, overlay, red, green, blue, alpha);
        anchor.render(poseStack, vertexConsumers, light, overlay, red, green, blue, alpha);
        for (ModelPart outer : outers) {
            outer.render(poseStack, vertexConsumers, light, overlay, red, green, blue, alpha);
        }
        poseStack.popPose();
    }
}
