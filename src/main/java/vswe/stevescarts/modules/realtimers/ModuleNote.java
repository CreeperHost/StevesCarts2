...
public class ModuleNote extends ModuleBase {
    ...
    public void drawForeground(GuiGraphicsExtractor GuiGraphicsExtractor, GuiMinecart gui) {
        ...
        for (int i = getScrollY(); i < Math.min(tracks.size(), getScrollY() + tracksInView); ++i) {
            ...
            for (int j = getScrollX(); j < Math.min(track.notes.size(), getScrollX() + notesInView); ++j) {
                ...
                note.drawText(GuiGraphicsExtractor, gui, i - getScrollY(), j - getScrollX());
            }
        }
    }
    ...
    public void drawBackground(GuiGraphicsExtractor GuiGraphicsExtractor, GuiMinecart gui, final int x, final int y) {
        ...
        for (int i = getScrollY(); i < Math.min(tracks.size(), getScrollY() + tracksInView); ++i) {
            ...
            for (int j = getScrollX(); j < Math.min(track.notes.size(), getScrollX() + notesInView); ++j) {
                final Note note = track.notes.get(j);
                note.draw(GuiGraphicsExtractor, texture, gui, x, y, i - getScrollY(), j - getScrollX());
            }
        }
        for (final Button button : buttons) {
            button.draw(GuiGraphicsExtractor, texture, gui, x, y);
        }
        if (tooLongTrack) {
            drawImage(GuiGraphicsExtractor, texture, gui, scrollXrect, 48, 0);
            int[] marker = getMarkerX();
            drawImage(GuiGraphicsExtractor, texture, gui, marker, 148, 1);
            if (veryLongTrack) {
                marker = getMarkerXTune();
                drawImage(GuiGraphicsExtractor, texture, gui, marker, 153, 1);
            }
        } else {
            drawImage(GuiGraphicsExtractor, texture, gui, scrollXrect, 48, 16);
        }
        if (tooTallModule) {
            drawImage(GuiGraphicsExtractor, texture, gui, scrollYrect, 0, 48);
            final int[] marker = getMarkerY();
            drawImage(GuiGraphicsExtractor, texture, gui, marker, 1, 148);
        } else {
            drawImage(GuiGraphicsExtractor, texture, gui, scrollYrect, 16, 48);
        }
    }
        ...
        for (int i = getScrollY(); i < Math.min(tracks.size(), getScrollY() + tracksInView); ++i) {
            ...
            for (int j = getScrollX(); j < Math.min(track.notes.size(), getScrollX() + notesInView); ++j) {
                ...
                if (note.instrumentId != 0) {
                    drawStringOnMouseOver(GuiGraphicsExtractor, gui, note.toString(), x, y, note.getBounds(i - getScrollY(), j - getScrollX()));
                }
            }
        }
        for (final Button button : buttons) {
            if (button.text != null && button.text.length() > 0) {
                button.overlay(GuiGraphicsExtractor, gui, x, y);
            }
        }
    }
    ...
    private class TrackButton extends Button {
        ...
        public void draw(GuiGraphicsExtractor GuiGraphicsExtractor, Identifier texture, GuiMinecart gui, final int x, final int y) {
            if (isValid()) {
                super.draw(GuiGraphicsExtractor, texture, gui, x, y);
            }
        }

        @Override
        public void overlay(GuiGraphicsExtractor GuiGraphicsExtractor, GuiMinecart gui, final int x, final int y) {
            if (isValid()) {
                super.overlay(GuiGraphicsExtractor, gui, x, y);
            }
        }
    ...
    private class Button {
        ...
        public void overlay(GuiGraphicsExtractor GuiGraphicsExtractor, GuiMinecart gui, final int x, final int y) {
            drawStringOnMouseOver(GuiGraphicsExtractor, gui, text, x, y, getRect());
        }
        ...
        public void draw(GuiGraphicsExtractor GuiGraphicsExtractor, Identifier texture, GuiMinecart gui, final int x, final int y) {
            ...
            drawImage(GuiGraphicsExtractor, texture, gui, getRect(), srcX, srcY);
            if (imageID != -1) {
                drawImage(GuiGraphicsExtractor, texture, gui, getRect(), imageID * 16, 32);
            }
        }
    }
    ...
    private class Note {
        ...
        public void drawText(GuiGraphicsExtractor GuiGraphicsExtractor, GuiMinecart gui, final int trackID, final int noteID) {
            if (instrumentId == 0) {
                return;
            }
            ...
            drawString(GuiGraphicsExtractor, gui, str, rect[0] + 3, rect[1] + 6, instrumentColors[instrumentId]);
        }

        public void draw(GuiGraphicsExtractor GuiGraphicsExtractor, Identifier texture, GuiMinecart gui, final int x, final int y, final int trackID, final int noteID) {
            int srcX = 0;
            if (instrumentId == 0) {
                srcX += 16;
            }
        ...
        public void play(final float volume) {
            ...
            if (!getCart().level().isClientSide()) {
                ...
                getCart().level().addParticle(ParticleTypes.NOTE, getCart().x() + oZ + 0.5, getCart().y() + 1.2, getCart().z() + oX + 0.5, pitch / 24.0, 0.0, 0.0);
