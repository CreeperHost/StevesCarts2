...
public class ModuleBrake extends ModuleAddon implements ILeverModule {
    ...
    private final int[] startstopRect;
    private final int[] turnbackRect;
    ...
    public void drawBackground(GuiGraphicsExtractor GuiGraphicsExtractor, GuiMinecart gui, final int x, final int y) {
        ...
        drawButton(GuiGraphicsExtractor, texture, gui, x, y, startstopRect, isForceStopping() ? 2 : 1);
        drawButton(GuiGraphicsExtractor, texture, gui, x, y, turnbackRect, 0);
    }

    private void drawButton(GuiGraphicsExtractor GuiGraphicsExtractor, Identifier texture, GuiMinecart gui, final int x, final int y, final int[] coords, final int imageID) {
        if (inRect(x, y, coords)) {
            drawImage(GuiGraphicsExtractor, texture, gui, coords, 0, coords[3]);
        } else {
            drawImage(GuiGraphicsExtractor, texture, gui, coords, 0, 0);
        }
        final int srcY = coords[3] * 2 + imageID * (coords[3] - 2);
        drawImage(GuiGraphicsExtractor, texture, gui, coords[0] + 1, coords[1] + 1, 0, srcY, coords[2] - 2, coords[3] - 2);
    }
