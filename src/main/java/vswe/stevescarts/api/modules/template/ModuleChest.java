...
public abstract class ModuleChest extends ModuleStorage {
    ...
    protected void handleChest() {
        ...
        if (isChestActive() && lidClosed() && playChestSound()) {
            getCart().level().playSound(null, getCart().blockPosition(), SoundEvents.CHEST_OPEN, SoundSource.PLAYERS, 0.5F, getCart().level().getRandom().nextFloat() * 0.1f + 0.9f);
        }
        if (isChestActive() && chestAngle < chestFullyOpenAngle()) {
            ...
            if (chestAngle < 1.1780972450961724 && lastAngle >= 1.1780972450961724 && playChestSound()) {
                getCart().level().playSound(null, getCart().blockPosition(), SoundEvents.CHEST_CLOSE, SoundSource.PLAYERS, 0.5F, getCart().level().getRandom().nextFloat() * 0.1f + 0.9f);
            }
            if (chestAngle < 0.0f) {
                chestAngle = 0.0f;
            }
