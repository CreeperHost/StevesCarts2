...
public class Tile
{
    ...
    public void draw(GuiGraphics guiGraphics, ResourceLocation texture, ArcadeSweeper game, final GuiMinecart gui, final int x, final int y, final int mx, final int my)
    {
        ...
        {
            game.getModule().drawImage(guiGraphics, texture, gui, rect, 30, 0);
        }
        else
        {
            ...
            game.getModule().drawImage(guiGraphics, texture, gui, rect, u, 0);
            if (isOpen() && nearbyCreepers != 0)
            {
                game.getModule().drawImage(guiGraphics, texture, gui, x + 1, y + 1, (nearbyCreepers - 1) * 8, 11, 8, 8);
            }
            if (state == TILE_STATE.FLAGGED)
            {
                if (!game.isPlaying && !isCreeper())
                {
                    game.getModule().drawImage(guiGraphics, texture, gui, x + 1, y + 1, 16, 20, 8, 8);
                }
                else
                {
                    game.getModule().drawImage(guiGraphics, texture, gui, x + 1, y + 1, 0, 20, 8, 8);
                }
            }
            else if (state == TILE_STATE.MARKED)
            {
                game.getModule().drawImage(guiGraphics, texture, gui, x + 1, y + 1, 8, 20, 8, 8);
            }
        }
    }
