...
public class ArcadeTracks extends ArcadeGame {
    ...
    private final int[] unlockedLevels;
    private final Cart player;
    private final Cart enderman;
    ...
    private final ArrayList<ScrollableList> lists;
    private boolean storySelected;
    private final ScrollableList storyList;
    private final ScrollableList mapList;
    private final ScrollableList userList;
...
//                ArcadeGame.playSound(SoundHandler.WIN, 1.0f, 1.0f);
    ...
    public void drawForeground(GuiGraphicsExtractor GuiGraphicsExtractor, GuiMinecart gui) {
        if (isSaveMenuOpen) {
            ...
            for (final ScrollableList list : lists) {
                list.drawForeground(GuiGraphicsExtractor, gui);
            }
            ...
            for (final LevelMessage message : currentMap.getMessages()) {
                if (message.isVisible(isRunning, isRunning && player.getDireciotn() == TrackOrientation.DIRECTION.STILL, isRunning && isItemTaken)) {
                    getModule().drawSplitString(GuiGraphicsExtractor, gui, message.getMessage(), 9 + message.getX() * 16, 9 + message.getY() * 16, message.getW() * 16, 4210752);
                }
            }
            if (isUsingEditor()) {
    ...
    public void drawBackground(GuiGraphicsExtractor GuiGraphicsExtractor, GuiMinecart gui, final int x, final int y) {
        if (!isSaveMenuOpen && isMenuOpen) {
            ...
            getModule().drawImage(GuiGraphicsExtractor, texture, gui, getMenuArea(), 0, 0);
            for (int i = 0; i < 3; ++i) {
                ...
                if (!hidden) {
                    getModule().drawImage(GuiGraphicsExtractor, texture, gui, rect[0], rect[1] + rect[3], 0, active ? 114 : 113, rect[2], 1);
                }
            }
            for (final ScrollableList list : lists) {
                list.drawBackground(GuiGraphicsExtractor, texture, gui, x, y);
            }
            ...
            if (isUsingEditor() && !isRunning) {
                for (int i = 0; i < trackMap.length; ++i) {
                    for (int j = 0; j < trackMap[0].length; ++j) {
                        getModule().drawImage(GuiGraphicsExtractor, texture, gui, 5 + i * 16, 5 + j * 16, 16, 128, 16, 16);
                    }
                }
            }
...
//                getModule().drawImage(GuiGraphicsExtractor, texture, gui, getTrackArea(track.getX(), track.getY()), 16 * track.getU(), 16 * track.getV(), track.getRotation());
            ...
            if (isUsingEditor()) {
                if (editorDetectorTrack != null && !isRunning) {
                    editorDetectorTrack.drawOverlay(GuiGraphicsExtractor, texture, getModule(), gui, editorDetectorTrack.getX() * 16 + 8, editorDetectorTrack.getY() * 16 + 8, isRunning);
                    getModule().drawImage(GuiGraphicsExtractor, texture, gui, 5 + editorDetectorTrack.getX() * 16, 5 + editorDetectorTrack.getY() * 16, 32, 128, 16, 16);
                }
            } else {
                for (final Track track : tracks) {
                    track.drawOverlay(GuiGraphicsExtractor, texture, getModule(), gui, x, y, isRunning);
                }
            }
            if (!isItemTaken) {
                int itemIndex = 0;
                ...
                getModule().drawImage(GuiGraphicsExtractor, texture, gui, 5 + itemX * 16, 5 + itemY * 16, 16 * itemIndex, 240, 16, 16);
            }
            for (final Cart cart : carts) {
...
//                getModule().drawImage(GuiGraphicsExtractor, texture, gui, x - 8, y - 8, 16 * editorTrack.getU(), 16 * editorTrack.getV(), 16, 16, editorTrack.getRotation());
            ...
            if (isSaveMenuOpen) {
                ...
                getModule().drawImage(GuiGraphicsExtractor, texture, gui, rect2, 0, 144);
            }
        }
        Identifier texture = ResourceHelper.getResource(ArcadeTracks.textureGame);
        for (int i = 0; i < 14; ++i) {
            if (isButtonVisible(i)) {
                ...
                final int srcY = i * 16;
                getModule().drawImage(GuiGraphicsExtractor, texture, gui, rect, srcX, srcY);
            }
        }
    }
    ...
    public void drawMouseOver(GuiGraphicsExtractor GuiGraphicsExtractor, GuiMinecart gui, final int x, final int y) {
        for (int i = 0; i < 14; ++i) {
            if (!isButtonDisabled(i) && isButtonVisible(i)) {
                getModule().drawStringOnMouseOver(GuiGraphicsExtractor, gui, getButtonText(i), x, y, getButtonArea(i));
            }
        }
    }
