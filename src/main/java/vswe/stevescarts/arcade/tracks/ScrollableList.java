...
public class ScrollableList {
    private final int x;
    private final int y;
    private final ArcadeTracks game;
    ...
    public void drawBackground(GuiGraphicsExtractor GuiGraphicsExtractor, Identifier texture, GuiMinecart gui, final int x, final int y) {
        if (!isVisible()) {
            return;
        }
        final int[] menu = game.getMenuArea();
        game.getModule().drawImage(GuiGraphicsExtractor, texture, gui, menu[0] + this.x, menu[1] + this.y, 0, 192, 132, 64);
        for (int i = 0; i < items.size(); ++i) {
            ...
            if (rect[3] > 0) {
                ...
                game.getModule().drawImage(GuiGraphicsExtractor, texture, gui, rect, 146, srcY);
                if (i == selectedIndex) {
                    game.getModule().drawImage(GuiGraphicsExtractor, texture, gui, rect, 146, borderSrcY);
                }
            }
        }
        final int[] area = getScrollArea();
        game.getModule().drawImage(GuiGraphicsExtractor, texture, gui, area[0], area[1] + scrollPosition, 132, 256 - ((items.size() >= 4) ? 32 : 16), 14, 16);
    }
    ...
    public void drawForeground(GuiGraphicsExtractor GuiGraphicsExtractor, GuiMinecart gui) {
        ...
        for (int i = 0; i < items.size(); ++i) {
            ...
            if (rect[4] >= -5) {
                if (rect[4] <= 48) {
                    game.getModule().drawString(GuiGraphicsExtractor, gui, (items.get(i) == null) ? "<???>" : items.get(i), x, y, 4210752);
                }
            }
        }
    }
