...
{
    public static final DeferredRegister<EntityDataSerializer<?>> SERIAL_REGISTER = DeferredRegister.create(ForgeRegistries.Keys.ENTITY_DATA_SERIALIZERS, Constants.MOD_ID);

    public static void init(IEventBus bus) {
        SERIAL_REGISTER.register(bus);
    }

    public static RegistryObject<EntityDataSerializer<int[]>> INT_ARRAY = SERIAL_REGISTER.register("int_array", () -> EntityDataSerializer.simple(FriendlyByteBuf::writeVarIntArray, FriendlyByteBuf::readVarIntArray));
    public static RegistryObject<EntityDataSerializer<BoolArray>> BOOL_ARRAY = SERIAL_REGISTER.register("bool_array", () -> EntityDataSerializer.simple((buf, bools) -> bools.write(buf), BoolArray::read));
    public static RegistryObject<EntityDataSerializer<ShortArray>> SHORT_ARRAY = SERIAL_REGISTER.register("short_array", () -> EntityDataSerializer.simple((buf, shorts) -> shorts.write(buf), ShortArray::read));
    public static RegistryObject<EntityDataSerializer<EnchantmentData>> ENCHANT_DATA = SERIAL_REGISTER.register("enchant_data", () -> EntityDataSerializer.simple((buf, data) -> data.write(buf), EnchantmentData::read));
